...
static int chanmap_ana1[] = { 0,2,4,6,8,10,12,14};
static int chanmap_ana2[] = { 1,3,5,7,9,11,13,15};
...
static inline void wctdm_transmitprep(struct wctdm *wc, u32 *writesamples)
{
	...
	for (x=0;x<DAHDI_CHUNKSIZE;x++) {
		...
		writechunk[chanmap_ana1[x]] = 0;
		writechunk[chanmap_ana2[x]] = 0;
		if (wc->cardflag & (1 << 7))
			writechunk[chanmap_ana1[x]] |= (wc->chans[7]->writechunk[x]);
		if (wc->cardflag & (1 << 6))
			writechunk[chanmap_ana1[x]] |= (wc->chans[6]->writechunk[x] << 8);
		if (wc->cardflag & (1 << 5))
			writechunk[chanmap_ana1[x]] |= (wc->chans[5]->writechunk[x] << 16);
		if (wc->cardflag & (1 << 4))
			writechunk[chanmap_ana1[x]] |= (wc->chans[4]->writechunk[x] << 24);

		if (wc->cardflag & (1 << 3))
			writechunk[chanmap_ana2[x]] |= (wc->chans[3]->writechunk[x]);
		if (wc->cardflag & (1 << 2))
			writechunk[chanmap_ana2[x]] |= (wc->chans[2]->writechunk[x] << 8);
		if (wc->cardflag & (1 << 1))
			writechunk[chanmap_ana2[x]] |= (wc->chans[1]->writechunk[x] << 16);
		if (wc->cardflag & (1 << 0))
			writechunk[chanmap_ana2[x]] |= (wc->chans[0]->writechunk[x] << 24);
	}
}
...
static inline void wctdm_receiveprep(struct wctdm *wc, u32 *readsamples)
{
	...
	for (x=0;x<DAHDI_CHUNKSIZE;x++) {
		if (wc->cardflag & (1 << 7))
			wc->chans[7]->readchunk[x] = (readchunk[chanmap_ana2[x]]) & 0xff;
		if (wc->cardflag & (1 << 6))
			wc->chans[6]->readchunk[x] = (readchunk[chanmap_ana2[x]] >> 8) & 0xff;
		if (wc->cardflag & (1 << 5))
			wc->chans[5]->readchunk[x] = (readchunk[chanmap_ana2[x]] >> 16) & 0xff;
		if (wc->cardflag & (1 << 4))
			wc->chans[4]->readchunk[x] = (readchunk[chanmap_ana2[x]] >> 24) & 0xff;

		if (wc->cardflag & (1 << 3))
			wc->chans[3]->readchunk[x] = (readchunk[chanmap_ana1[x]]) & 0xff;
		if (wc->cardflag & (1 << 2))
			wc->chans[2]->readchunk[x] = (readchunk[chanmap_ana1[x]] >> 8) & 0xff;
		if (wc->cardflag & (1 << 1))
			wc->chans[1]->readchunk[x] = (readchunk[chanmap_ana1[x]] >> 16) & 0xff;
		if (wc->cardflag & (1 << 0))
			wc->chans[0]->readchunk[x] = (readchunk[chanmap_ana1[x]] >> 24) & 0xff;
	}
...
static void set_daisychain(struct wctdm *wc, int cs) {
	__wctdm_setcard(wc, cs);
	___spi_write(0x0, wc);
	___spi_write(0x80, wc);
}
...
static void __wctdm_setreg(struct wctdm *wc, int card, unsigned char reg, unsigned char value)
{
	...
	if (wc->modtype[card] == MOD_TYPE_FXO) {
        cs = ((cs & 0x55) <<  1)  | ((cs >>  1) & 0x55);
        cs = ((cs & 0x33) <<  2)  | ((cs >>  2) & 0x33);
		___spi_write(0x20 | cs, wc);
		___spi_write(reg & 0x7f, wc);
	}
	else if (wc->modtype[card] == MOD_TYPE_FXS) {
		___spi_write(0xff & (1<<cs), wc);
		___spi_write(reg & 0x7f, wc);
	}
...
static int wctdm_hardware_init(struct wctdm *wc)
{
	...
	/* we need sync the clock first for 2 TDM bus*/
	cpld_write(CLOCK_SYNC, wc);
	printk("TDM bus clock synced with SwitchPi CPLD core!");
	fx_auto_detect(wc);
	reset_spi(wc, 1);
...
#if 1
		if ( (wc->modtype[x] == MOD_TYPE_FXS) & (x == 4) ) {
			set_daisychain(wc, x);
		}
		udelay (20);
...
static int wctdm_init_one(void)
{
	...
	if (0) {
		...
		if (wc) {
			...
			gwc = wc;
			if (cardcount != 0) cpld_write(LED_ON, wc);
			else cpld_write(LED_OFF, wc);
